...
static int tx_worker(void *arg) {
  ...
  while (tx_count < ctx->packet_count) {
    ...
    for (int i = 0; i < burst_size; i++) {
      ...
      //
      // For the payload, you are free to fill it however you want.
      // The simplest way is to just memset it to a byte value.
      //
      // You can calculate the payload length by subtracting the size of the
      // headers from the total length.
      //
...
static int rx_worker(void *arg) {
  ...
  while (rx_count < ctx->packet_count) {
    ...
    //
    uint16_t nb_rx = 0;
    return 1; // remove this after solving
